...
public class CaveBuilder {
	...
	private CaveBuilder createRegions(){
		...
		for (int z = 0; z < depth; z++){
			for (int x = 0; x < width; x++){
				for (int y = 0; y < height; y++){
					if (tiles[x][y][z] != Tile.CAVE_WALL && regions[x][y][z] == 0){
		...
		while (!open.isEmpty()){
			...
			for (Point neighbor : p.neighbors8()){
				...
				if (regions[neighbor.x][neighbor.y][neighbor.z] > 0
						|| tiles[neighbor.x][neighbor.y][neighbor.z] == Tile.CAVE_WALL)
					continue;
	...
	private CaveBuilder addExitStairs() {
		int x = -1;
		int y = -1;
